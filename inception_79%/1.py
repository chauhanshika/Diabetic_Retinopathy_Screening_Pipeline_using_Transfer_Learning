...
IMG_SIZE = (299, 299)
BATCH_SIZE = 8   # use batch size 8 globally
AUTOTUNE = tf.data.AUTOTUNE
SEED = 123
NUM_EPOCHS = 51
INITIAL_EPOCHS = 51          # more epochs for higher accuracy (still frozen-backbone only)
DEFAULT_N_QUBITS = 4
DEFAULT_Q_LAYERS = 2
USE_QUANTUM = False               # keep False by default; enable only for experiments
CACHE_DATASETS = True
MIXUP_ALPHA = 0.15                # MixUp intensity (0 disables MixUp)
LABEL_SMOOTHING = 0.02            # lighter label smoothing to preserve peak accuracy
WEIGHT_DECAY = 1e-4               # AdamW weight decay
DENSE_AFTER_Q = 512               # larger head
DROPOUT_RATE = 0.4                # slightly lower dropout to fit better
EARLYSTOP_PATIENCE = 15
EXIT_AFTER_TRAIN = False         # run evaluation to generate confusion matrix
THRESHOLD_ACC = 0.999             # effectively disabled; rely on EarlyStopping
FINE_TUNE_EPOCHS = 15             # extra epochs for fine-tuning
FINE_TUNE_AT = 80                 # unfreeze last N layers of the base model

# Auto-tune core regularization according to batch size
if BATCH_SIZE <= 8:
    MIXUP_ALPHA = 0.12
...
data_augmentation = tf.keras.Sequential([
    ...
    layers.CenterCrop(int(IMG_SIZE[0] * 0.9), int(IMG_SIZE[1] * 0.9)),
...
def build_model(base_model, n_qubits=DEFAULT_N_QUBITS, q_layers=DEFAULT_Q_LAYERS, lr=1e-4, dense_after_q=DENSE_AFTER_Q, use_quantum=USE_QUANTUM, dropout_rate=DROPOUT_RATE, label_smoothing=LABEL_SMOOTHING, use_focal_loss=False):
    ...
    reference_batch = 32.0
    scaled_lr = lr * (float(BATCH_SIZE) / reference_batch)
    optimizer = make_optimizer('adamw', scaled_lr)
...
def run_keras_tuner_search(train_ds, val_ds, base_model, class_weight, max_trials=8, epochs_per_trial=6):
    ...
    tuner.search(train_ds, validation_data=val_ds, epochs=epochs_per_trial, callbacks=[], class_weight=class_weight)
...
# Callbacks
checkpoint_filepath = 'best_quantum_inception_cpu.keras'
callbacks_initial = [
...
with strategy.scope():
    if steps_per_epoch:
        ...
        init_lr_schedule = WarmUpCosine(
            base_lr=chosen_lr * (float(BATCH_SIZE) / 32.0),
            total_steps=init_total_steps,
            warmup_steps=init_warmup_steps,
            min_lr=max(1e-7, (chosen_lr * (float(BATCH_SIZE) / 32.0)) * 0.1),
        )
        try:
            Opt = tf.keras.optimizers.experimental.AdamW
        ...
        try:
            Opt = tf.keras.optimizers.experimental.AdamW
            init_optimizer = Opt(learning_rate=chosen_lr * (float(BATCH_SIZE) / 32.0), weight_decay=chosen_weight_decay)
        except Exception:
            init_optimizer = tf.keras.optimizers.Adam(learning_rate=chosen_lr * (float(BATCH_SIZE) / 32.0))
...
# Save final model
model.save('quantum_inception_trained_cpu.keras')
print('Saved trained model to quantum_inception_trained_cpu.keras')
...
for batch_images, batch_labels in val_ds:
    ...
    # Test-time augmentation: average original and horizontally flipped predictions
    flipped_images = tf.image.flip_left_right(batch_images)
    combined = tf.concat([batch_images, flipped_images], axis=0)
    preds_combined = model.predict(combined, verbose=0)
    half = preds_combined.shape[0] // 2
    preds_avg = (preds_combined[:half] + preds_combined[half:]) / 2.0
    y_prob_list.append(preds_avg)
